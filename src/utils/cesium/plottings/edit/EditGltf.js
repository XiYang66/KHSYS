 ...
 * 编辑文本方法
 */
import { transformCartesianToWGS84 } from "../thirdPart/Coordinate";

const EditGltf = (viewer, callBack, entity, handler, collection) => {
  let editObj = new EditGltf4Entity(viewer, entity, 1, 1);
  editObj.editRtation();
...
class EditGltf4Entity {
  ...
  initParam() {
    ...
    this._params.tx = originDegree.x;
    this._params.ty = originDegree.y;
    this._params.tz = originDegree.z;
  ...
  returnGltfCentorDegree(gltf) {
    ...
    const originDegree = transformCartesianToWGS84(gltfCenter);
    return originDegree;
  }
  /**
   * 开始旋转编辑
  ...
  editRtation() {
    ...
    this.createCircle(
      option.originDegree.x,
      option.originDegree.y,
      option.originDegree.z,
      option.length
    );
  }
  createCircle(x, y, height, radius) {
    const position = [];
    ...
    const matrix = Cesium.Transforms.eastNorthUpToFixedFrame(
      new Cesium.Cartesian3.fromDegrees(x, y, height)
    );
    //绕Z轴
  ...
  editTranslation() {
    ...
    this.initLineArrow(option.originDegree, targetDegree);
  }
    ...
    $this._handler.setInputAction(function (event) {
      ...
      if (
        ...
        if (downPos && Cesium.defined(downPos)) {
          ...
          _ry = 0;
          _rz = 0;
          const downDegree = transformCartesianToWGS84(downPos);
          $this._handler.setInputAction(function (movement) {
            let endPos = viewer.scene.camera.pickEllipsoid(
              movement.endPosition,
              viewer.scene.globe.ellipsoid
            );
            const endDegree = transformCartesianToWGS84(endPos);
            ...
            switch ($this._currentPick._name) {
              case "model_edit_xArrow":
                _tx = endDegree.x - downDegree.x;
                break;
              case "model_edit_yArrow":
                _ty = endDegree.y - downDegree.y;
                break;
  ...
  getTransPostion(originPosition, translateCartesian) {
    ...
    const result = transformCartesianToWGS84(originPosition);
    return result;
  }
  removeCoordArrows() {
    if (this._coordArrows) {
