 ...
 * @returns
 */
function calculateTwoPoints(anchorpoints, type = 1) {
  let ratio = 6;
 ...
 * @returns
 */
const calculateRectanglePoints = (points) => {
 ...
 * @private
 */
const computeAssemblePoints = (anchorpoints) => {
  let points = [];
...
const calculateSwallowPoints = (anchorpoints, type) => {
  let points = [];
  switch (type) {
    case 1:
 ...
 * @private
 */
const computeSwallowtailArrow = (anchorpoints, swallow = true) => {
 ...
 * @param anchorpoints
 */
const computeSwallowtailArrow2 = (anchorpoints) => {
  let p0 = transformCartesianToWGS84(anchorpoints[0]);
  let p1 = anchorpoints[1]
    ? transformCartesianToWGS84(anchorpoints[1])
    : { x: p0.x + 0.00001, y: p0.y + 0.00001, z: p0.z };
 ...
 * @returns
 */
const calculatePincerArrowPoint = (points) => {
...
const calculateFlagPoints = (points, type = 0) => {
 ...
 * @returns
 */
function getFlagPoints(points, type = 0) {
  type = type ? type : 0;
  let components = [];
  // 至少需要两个控制点
  if (points.length > 1) {
 ...
 * @returns
 */
const calculateFormationPoints = (anchorpoints, type) => {
 ...
 * @returns
 */
const calculateBowPoints = (anchorpoints) => {
  let pos0 = transformCartesianToWGS84(anchorpoints[0]);
  let pos1 = anchorpoints[1]
    ? transformCartesianToWGS84(anchorpoints[1])
    : { x: pos0.x + 0.00001, y: pos0.y + 0.00001, z: pos0.z };
 ...
 * @returns
 */
const computeRoundedRectanglePoints = (anchorpoints) => {
  let pos0 = transformCartesianToWGS84(anchorpoints[0]);
  let pos1 = anchorpoints[1]
    ? transformCartesianToWGS84(anchorpoints[1])
    : { x: pos0.x + 0.00001, y: pos0.y + 0.00001, z: pos0.z };
...
const calculateSectorPoints = (points) => {
